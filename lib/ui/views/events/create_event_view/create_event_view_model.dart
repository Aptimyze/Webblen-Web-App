...
class CreateEventViewModel extends ReactiveViewModel {
  ...
  bool formIsValid() {
    bool isValid = false;
    if (fileToUploadByteMemory.isEmpty && event.imageURL == null) {
  ...
  Future<bool> uploadData() async {
    ...
    if (fileToUploadByteMemory.isNotEmpty) {
      String imageURL = await _firestoreStorageService.uploadImage(imgFile: fileToUpload, storageBucket: 'images', folderName: 'events', fileName: event.id!);
      if (imageURL.isEmpty) {
        _customDialogService.showErrorDialog(
  ...
  ///NAVIGATION
  navigateBack() async {
    _customDialogService.showCancelContentDialog(isEditing: isEditing, contentType: "event");
  }
